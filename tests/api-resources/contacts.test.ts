...
describe('resource contacts', () => {
  ...
  // Mock server tests are disabled
  test.skip('retrieveMessageSummary', async () => {
    const responsePromise = client.contacts.retrieveMessageSummary('6ba7b810-9dad-11d1-80b4-00c04fd430c8');
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  // Mock server tests are disabled
  test.skip('retrieveMessageSummary: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.contacts.retrieveMessageSummary(
        '6ba7b810-9dad-11d1-80b4-00c04fd430c8',
        { 'x-profile-id': '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e' },
        { path: '/_stainless_unknown_path' },
      ),
